...
public class PPSlideGenerator
{
    ...
    // Consts
    private static readonly float _radius = 2.2f;
    private static readonly Vector3 _center = new Vector3(_radius, 0, 0);

    public PPSlideGenerator(Vector3 start, Vector3 end)
    {
        _start = start;
        _end = end;
        ...
        if (float.IsNaN(_intersectB.x)) // Fix for 1ppD3 can't find a tangent point
            _intersectB = _end;
