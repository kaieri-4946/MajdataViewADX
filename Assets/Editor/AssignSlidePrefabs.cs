...
public class AssignSlidePrefabs : Editor
{
    ...
    {
        var targetScript = Selection.activeGameObject.GetComponent<JsonDataLoader>();
        if (targetScript == null)
        {
            Debug.LogWarning($"Selected object doesn't contain a {nameof(JsonDataLoader)} script");
            return;
        }
        ...
        var slideMap = JsonDataLoader.GetSlidePrefabMap()
        ...
        foreach (var slide in slideMap)
        {
            ...
            {
                Debug.LogError("JsonDataLoader SlidePrefabMap is not contiguous");
                return;
            }
